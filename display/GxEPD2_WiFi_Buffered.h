...
extern GxEPD2_3C<GxEPD2_420_Z98c, GxEPD2_420_Z98c::HEIGHT> display;
...
while (client.connected()) {
  ...
  // Kiểm tra dòng phản hồi đầu tiên: HTTP/1.1 200 OK
  if (!connection_ok && line.startsWith("HTTP/")) {
    int status_code = line.substring(9, 12).toInt();
    connection_ok = (status_code == 200);
  ...
  // In các dòng header khác nếu chưa xác nhận connection_ok
  if (!connection_ok) {
    Serial.println(line);
  }

  // Dấu hiệu kết thúc phần header
  if (line == "\r") {
